...
class _ManagerQRScanPageState extends State<ManagerQRScanPage> {
  ...
  Future<void> handleScan(String qrData) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data["type"] == "student") {
          displayName = data["student"]["name"]?.toString() ?? "Student";
        } else if (data["type"] == "guest") {
          displayName = data["guest"]["name"]?.toString() ?? "Guest";
        }
        ...
        await _showVerificationResult(
          success: false,
          error: data["error"]?.toString() ?? "Failed to verify meal",
        );
      }
    } catch (e) {
      await _showVerificationResult(success: false, error: e.toString());
    }
    ...
    if (mounted) {
      setState(() {
        isProcessing = false;
        lastScanned = null; // Clear this so the same QR can be scanned if needed
      });
    }
  }
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            if (success) ...[
              _resultRow("Student", studentName ?? "Unknown"),
              const SizedBox(height: 8),
              const Text("ITEMS TO SERVE:", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.blueGrey)),
